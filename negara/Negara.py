...
class Negara:
    ...
    def plot_exports_imports_bar(self, df, title, savefig_name):
        # Ensure the DataFrame is sorted by index (e.g., country names)
        df = df.sort_index()

        # Define Morandi color palette for blue and red
        morandi_blue = '#8A9A9A'
        morandi_red = '#D36E70'
        ...
        # Create an array of indices for the x positions
        indices = range(len(df.index))

        # Plot Total Exports
        plt.bar([i - bar_width/2 for i in indices], df['Total_Exports'], 
                width=bar_width, label='Total Exports', color=morandi_blue)

        # Plot Total Imports
        plt.bar([i + bar_width/2 for i in indices], df['Total_Imports'], 
                width=bar_width, label='Total Imports', color=morandi_red)
        ...
        # Set the xticks to be the country names
        plt.xticks(indices, df.index, rotation=90)  # Rotate country names for readability

        # Set legend to be outside the plot
        plt.legend(title='Category', loc='upper left', bbox_to_anchor=(1, 1))  # Place legend outside to the right
